...
public class secondaryMemory : MonoBehaviour
{
    ...
    public bool TryAddBlockToSlot(GameObject slot, GameObject block)
    {
            ...
            (offsetSlots.Contains(slot) && blockType.blockType != BlockType.Type.Offset))
        {
            ...
            // Place the block above the player's head and make it follow the player
            block.transform.SetParent(playerMovementScript.transform);  // Attach block to player
            block.transform.position = playerMovementScript.holdPosition.position;  // Above player
            block.SetActive(true);

            return false;  // Return false to indicate the block couldn't be added to the slot
        }

        // Block is placed correctly in the slot
        slotToBlockMap[slot] = block;
        block.transform.SetParent(slot.transform);
        block.transform.localPosition = Vector3.zero;
        ...
        // After adding, validate all page-offset pairs
    ...
    public void ValidatePageOffsetPair(int index)
    {
        ...
        if (pageBlock != null && offsetBlock != null)
        {
            ...
            if (blockColorChangers.Count > index)
            {
                blockColorChangers[index].SetTargetValue(sum, targetValues[index]); // Color change happens here
            }
        }
        else
        {
